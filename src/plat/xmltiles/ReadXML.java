	...
	public static void Read(String XmlFile) {
		try {
			...
			for (int n = 0; n < tList.getLength(); n++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) { // Make sure it
																// is a node
					Element eElement = (Element) nNode; // Return the element as
														// var eElement

					System.out.println("The tile type is: "
							+ eElement.getAttribute("type")); // TEMP
				}
			}
		}
